...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        cur_x,cur_y=cell
        mine_counter=0
        neighbours=[(cur_x-1,cur_y),(cur_x+1,cur_y),(cur_x,cur_y-1),(cur_x,cur_y+1)]

        for node in neighbours:
            x,y=node
            if(0<=x<self.height and 0<=y<self.width):
                if node not in self.safes and node not in self.mines:
                    new_cells.add(node)
                elif node in self.mines:
                    mine_counter+=1
        
        new_sentence=Sentence(new_cells,count-mine_counter)
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                if sentence1 is sentence2:
                    ...
                    sentence3=Sentence(list(diffcells),sentence2.count-sentence1.count)
